...
public class UrlEncodedWebServiceController extends WebServiceController {
    ...
    @RequestMapping(value = "/message", method = RequestMethod.POST, headers = "Content-Type=application/x-www-form-urlencoded",produces = "application/x-www-form-urlencoded")
    public String message(@RequestHeader(HttpHeaders.AUTHORIZATION) String authorization, @RequestParam("request") String message, HttpServletRequest request,HttpServletResponse response) throws TransportClientException, MessageParserException, UserNotFoundException {
        try {
            ...
            String encodedResponseMessage = MessageUtils.encodeMedHistory(super.message(decodedMessage, criteria));
