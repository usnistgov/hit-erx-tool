...
public abstract class TransportController {
	...
	public TransportResponse populateMessage(TransportRequest request, HttpSession session, String domain) {
	...
	public String send(TransportRequest request, String message) throws TransportClientException {
		try {
			...
			String incoming = webServiceClient.send(message, request.getConfig().get("username"),
					request.getConfig().get("password"), request.getConfig().get("endpoint"), contentType);
	...
	protected Transaction saveTransaction(Long accountId, TestStep testStep, String incoming, String outgoing) {
		...
		if (transaction == null) {
			...
			logger.info("Transaction successfully saved: {accountId:" + accountId + ",testStepId:"
					+ testStep.getPersistentId() + ",incoming:" + incoming + ",outgoing:" + outgoing + "}");
		}
		return transaction;
	}
	...
	private Map<String, String> getSutInitiatorConfig(Long accountId, String PROTOCOL, String DOMAIN) {
		...
		if (taInitiator != null && taInitiator.containsKey("replaceSeparators")) {
			...
			logger.info("Adding the replaceSeparator parameter to the SUT config: "
					+ sutInitiator.get("replaceSeparators"));
		}
		return sutInitiator;
	}
